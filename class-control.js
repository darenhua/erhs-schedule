...
(async function main() {
  ...
  const adv_zoom = "https://zoom.us/";

  let calender = await fetch_calender();
  const zooms = await fetch_links();
  ...
  function update_daily() {
    ...
    function insert_card() {
      ...
      last_card.id = "";
      last_card.insertAdjacentHTML(
        "afterend",
        '<a target="_blank" class="card" id="card-l"> <div id="gym-crop" class="sgi"> <img src="assets/img/dumbbell.svg" alt="dumbbell" id="gym-graphic"> </div> <h5 class="class-label">SGI-2</h5> </a>'
      );
    }
  ...
  function update_s() {
    ...
    greeting_elem.innerHTML =
      hour > 11 ? date_str.greeting[2] : date_str.greeting[0];
    greeting_elem.innerHTML =
      hour === 11 ? date_str.greeting[1] : greeting_elem.innerHTML;
    ...
    function reset_styles() {
      for (i = 0; i < links_updated.length; i++) {
        links[i].style.background =
          "linear-gradient(180deg,rgba(255, 255, 255, 0.5) 0%,rgba(255, 255, 255, 0) 100%),#0e0f2f";
      }
    ...
    function current_class() {
      ...
      for (i = 0; i < classes_times.length - 1; i++) {
        ...
        if (startDate < date_obj && endDate > date_obj) {
          my_class = i;
          links_updated[i].style.background =
            "linear-gradient(180deg, #FFFFFF 0%, rgba(255, 255, 255, 0) 100%), #098A82";
      ...
      if (sgi) {
        if (!normal_schedule) {
          sgi_times =
            sgi.length > 2
              ? ["13:20:00", "13:50:00", "14:20:00", "14:50:00"]
              : ["13:20:00", "14:05:00", "14:50:00"];
          for (i = 0; i < sgi_times.length - 1; i++) {
            ...
            if (startDate < date_obj && endDate > date_obj) {
              ...
              links_updated[i + classes_times.length - 1].style.background =
                "linear-gradient(180deg, #FFFFFF 0%, rgba(255, 255, 255, 0) 100%), #098A82";
              class_is_over = false;
              break;
            }
